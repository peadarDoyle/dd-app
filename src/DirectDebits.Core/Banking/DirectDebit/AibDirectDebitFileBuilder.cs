...
{
    public class AibDirectDebitFileBuilder : DirectDebitFileBuilder
    {
        ...
        private static XElement CreatePayment(Batch batch, DateTime processingDate)
        {
            ...
            return new XElement(Ns1 + "PmtInf",
                    ...
                    new XElement(Ns1 + "PmtMtd", payment.PmtMtd),
                    ...
                    new XElement(Ns1 + "PmtTpInf",
                        new XElement(Ns1 + "SvcLvl",
                            new XElement(Ns1 + "Cd", payment.PmtTpInfCd)
                        ),
                        new XElement(Ns1 + "LclInstrm",
                            new XElement(Ns1 + "Cd", payment.LclInstrmCd)
                        ),
                        new XElement(Ns1 + "SeqTp", payment.SeqTp)
                    ),
        ...
        private static IEnumerable<XElement> CreateTransactions(Batch batch, IList<BankAgent> bankAgents)
        {
            ...
            for (int i = 0; i < allocationsGroupedByAccount.Count(); i++)
            {
                ...
                string authId = batch.Organisation.GetSettings(batch.BatchType).AuthId;
                int transactionNum = i + 1;
                var transaction = new TransactionData(transactionNum, authId, bankAgent, allocations);
                        ...
                        new XElement(Ns1 + "CdtrSchmeId",
                            new XElement(Ns1 + "Id",
                                new XElement(Ns1 + "PrvtId",
                                    new XElement(Ns1 + "Othr",
                                        new XElement(Ns1 + "Id", transaction.AuthId),
                                        new XElement(Ns1 + "SchmeNm",
                                            new XElement(Ns1 + "Prtry", transaction.SchmeName)
                                        )
                                    )
                                )
                            )
